# my_recipe_book
학습을 통해 배우고 다음에도 사용할 만한 파이프라인을 체계적으로 정리합니다.

대회에 참여해보고 그 대회에서 배운 내용을 정리하는 공간입니다.

사람들이 공유한 상위권들의 코드들을 보며 정리합니다.



현재 참여해본 대회 :
1. 자갈 분류
2. k리그 패스길
3. 제주도 도로 교통량 예측
4. lg 식음업장 메뉴 수요 예측
5. 모기 비행궤적 예측 ( 리더보드 0.72 1위 / 내 코드 0.6926 / 내 코드 기반 마지막 팀원 rotation 게이트 추가 업데이트 0.6992 / private 0.7002 12등 / 786 팀  ) <-- 여기서부터는 뭔가 따라가는 느낌은 남


논문 읽어보고 정리하기 or 리뷰하기 :
1. [https://app.notion.com/p/Deep-Reinforcement-Learning-for-Automated-Stock-Trading-An-Ensemble-Strategy-378ecb67088c80bc8876f5723ec992ef?source=copy_link] Deep Reinforcement Learning for Automated Stock Trading: An Ensemble Strategy
